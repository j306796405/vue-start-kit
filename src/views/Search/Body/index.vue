<template>
  <div class="search-body-wrapper" v-scroll="loadingMore">
    <div class="search-container">
      <div class="guess" v-if="searchList && searchList.length < 1">
        <div class="tip">猜你想搜的</div>
        <ul class="search-menu border-half cf">
          <li class="fl border-half" v-for="keyword in keywordList" @click="clickSearchBtn(keyword)">{{keyword}}</li>
        </ul>
      </div>
      <div class="search-box" v-else>
        ...
        <NoData v-if="searchHasMore"></NoData>
      </div>
    </div>
    <DefaultLoading v-show="searchIsLoading"></DefaultLoading>
  </div>
</template>
<script>
