<template>
  <article class="video-wrapper" v-scroll="loadingMore">
    ...
    <DefaultLoading v-show="videoIsLoading"></DefaultLoading>
    <NoMoreData v-if="videoHasMore"></NoMoreData>
  </article>
</template>
    ...
    methods: {
      ...
      loadingMore () {
        return new Promise(async (resolve, reject) => {
          await this.$store.dispatch('getVideoList', {pageindex: 1})
          resolve()
        })
      }
    },
    created () {
      this.$store.dispatch('getVideoList', {pageindex: 1})
    }
  }
</script>
